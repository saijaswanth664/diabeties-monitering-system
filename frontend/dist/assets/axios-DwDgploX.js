import { A as __exportAll } from "./index-gvqOpvu8.js";
...
var API_BASE_URL = typeof window !== "undefined" && (window.location.port === "5173" || window.location.hostname === "localhost") ? "http://127.0.0.1:8000" : "";
var MOCK_MODE = { active: false };
...
apiClient.interceptors.response.use((response) => response, async (error) => {
	...
	if ((!error.response || error.code === "ERR_NETWORK") && MOCK_MODE.active || MOCK_MODE.active) {
		...
		if (mockResponse) return Promise.resolve({
			data: mockResponse,
			status: 200,
			statusText: "OK",
			headers: {},
...
function getMockResponse(url, method, rawData) {
	...
	if (cleanUrl.includes("auth/signup")) return { message: "Registration successful. A 6-digit OTP has been sent to your Gmail." };
	if (cleanUrl.includes("auth/verify-otp")) return { message: "Email verified successfully. You can now login." };
	if (cleanUrl.includes("auth/login")) return {
		access_token: "mock_jwt_jsonwebtoken_token_sarah_jenkins",
		token_type: "bearer",
		full_name: "Dr. Sarah Jenkins",
		gmail: data?.username || "sarah.jenkins@gmail.com"
	};
	if (cleanUrl.includes("auth/forgot-password")) return { message: "Verification code sent. Use it to reset your password." };
	if (cleanUrl.includes("auth/reset-password")) return { message: "Password reset successfully. You can now login with your new password." };
	if (cleanUrl.includes("health/upload-health-data")) {
		const outcome = data.glucose > 140 || data.bmi > 30 ? 1 : 0;
		const prob = outcome === 1 ? .75 + Math.random() * .2 : .15 + Math.random() * .2;
		const glucoseDiff = outcome === 1 ? 25 : -35;
		return {
			record: {
				record_id: Math.floor(Math.random() * 1e3),
				month: data.month,
			...
			comparison: {
				glucose: {
					current: data.glucose,
					previous: data.glucose - glucoseDiff,
					difference: glucoseDiff,
					status: glucoseDiff > 0 ? "Increased (Risk)" : "Improved"
				},
				bmi: {
					current: data.bmi,
					previous: data.bmi - 2,
				...
				insulin: {
					current: data.insulin,
					previous: data.insulin,
					difference: 0,
					status: "Stable"
				},
			...
			analysis: {
				warnings: outcome === 1 ? ["WARNING: Elevated Blood Glucose (Glucose > 140 mg/dL). Indicates potential prediabetes or uncontrolled glycemic response.", "WARNING: Clinical Obesity (BMI >= 30). Increases cardiovascular risk, insulin resistance, and overall morbidity."] : [],
				recommendations: [
	...
	if (cleanUrl.includes("health/history")) return [
		{
			record_id: 101,
			month: "2026-03",
			glucose: 178,
	...
	if (cleanUrl.includes("reports/generate-report")) return {
	...
	if (cleanUrl.includes("reports/send-report")) return { message: "Clinical report successfully emailed to sarah.jenkins@gmail.com." };
	return null;
}
//#endregion
export { apiClient as n, MOCK_MODE as t };
